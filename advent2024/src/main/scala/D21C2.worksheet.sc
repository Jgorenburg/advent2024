...
var numpadDist = Map[(Char, Char), Long]()
var arrowpadDist = Array.fill(26)(Map[(Instruction, Instruction), Long]())

def getPathLen(path: List[Instruction], depth: Int): Long = {
...
for (i <- 1 to 25) {
  Instruction.values.foreach(start =>
    Instruction.values.foreach(end =>
      arrowpadDist(i)((start, end)) = uniquePermutations(
        getInstructions(arrowpadLoc(start), arrowpadLoc(end)),
        false,
        arrowpadLoc(start)
      ).map(getPathLen(_, i)).min
    )
  )
}
